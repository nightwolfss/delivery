...
public class ClienteDao extends Dao{
	...
	public String gravarCliente(Cliente cliente) {
		try {
			conectarBanco();
			stmt = con.prepareStatement("insert into cliente (endereco, nome, telefone, observacao, idDono) values (?,?,?,?,?)");
			...
			stmt.setInt(5, cliente.getIdDono());
			stmt.execute();
	...
	public ArrayList<Cliente> listaClientePorLogin(Integer idDono){
		try {
			ArrayList<Cliente> lista = new ArrayList<Cliente>();
			conectarBanco();
			stmt = con.prepareStatement("select * from cliente where idDono = ? order by nome asc");
